...
class UserControllerTest extends AbstractIntegrationTest {
    ...
    void searchUsers() throws UserException {
        ...
        ResponseEntity<List<UserDTO>> result = userController.searchUsers("Luke", authorization);
        assertThat(result.getStatusCode()).isEqualTo(HttpStatus.OK);
        assertThat(result.getBody()).containsExactly(luke.getBody());

        // Search by mail
        result = userController.searchUsers("skywalker@test.com", authorization);
        assertThat(result.getStatusCode()).isEqualTo(HttpStatus.OK);
        assertThat(result.getBody()).containsExactly(luke.getBody());
    }
    ...
    void searchUsersByName() throws UserException {
        ...
        String authorization = JwtConstants.TOKEN_PREFIX + response.token();
        ResponseEntity<UserDTO> luke = userController.getUserProfile(authorization);
        ResponseEntity<List<UserDTO>> result = userController.searchUsersByName("Luke Skywalker", authorization);
        assertThat(result.getBody()).containsExactly(luke.getBody());
    }
    ...
    void updateUser() throws UserException {
        ...
        assert response != null;
        String authorization = JwtConstants.TOKEN_PREFIX + response.token();
        UpdateUserRequestDTO request = new UpdateUserRequestDTO(mail, password, name, "19900000000");
